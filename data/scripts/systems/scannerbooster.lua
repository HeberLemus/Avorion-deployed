...
function getBonuses(seed, rarity)
    ...
    scanner = scanner + (rarity.value + 2) * 45 -- add +15% (worst rarity) to +105% (best rarity)

    -- add randomized percentage, span is based on rarity
    scanner = scanner + math.random() * ((rarity.value + 1) * 45) -- add random value between +0% (worst rarity) and +90% (best rarity)
    scanner = scanner / 100
