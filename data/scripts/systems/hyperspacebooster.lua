...
function getBonuses(seed, rarity)
    ...
    if enabled[StatsBonuses.HyperspaceReach] then
        reach = math.max(0, getInt(rarity.value, rarity.value * 3.0)) + 1
    end

    if enabled[StatsBonuses.HyperspaceCooldown] then
        cdfactor = 10 -- base value, in percent
        -- add flat percentage based on rarity
        cdfactor = cdfactor + (rarity.value + 1) * 5 -- add 0% (worst rarity) to +18% (best rarity)

        -- add randomized percentage, span is based on rarity
        cdfactor = cdfactor + math.random() * ((rarity.value + 1) * 5) -- add random value between 0% (worst rarity) and +18% (best rarity)
    ...
    if enabled[StatsBonuses.HyperspaceRechargeEnergy] then
        ...
        -- add flat percentage based on rarity
        efactor = efactor + (rarity.value + 1) * 4 -- add 0% (worst rarity) to +18% (best rarity)
