...
function getBonuses(seed, rarity, permanent)
    ...
    durability = (durability + (rarity.value * 0.03) + (0.03 * math.random())) * 1.5
...
function getEnergy(seed, rarity, permanent)
    ...
    return durability * 1.75 * 1000 * 1000 * 100
end

function getPrice(seed, rarity)
    local durability, rechargeTimeFactor = getBonuses(seed, rarity)
    local price = durability * 1000 * 50
...
function getTooltipLines(seed, rarity, permanent)
    ...
    if rechargeTimeFactor ~= 0 then
        table.insert(texts, {ltext = "Time Until Recharge"%_t, rtext = string.format("%+i%%", rechargeTimeFactor * 100), icon = "data/textures/icons/sands-of-time.png", boosted = permanent})
    end
