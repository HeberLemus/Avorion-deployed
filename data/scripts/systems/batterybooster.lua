...
function getBonuses(seed, rarity, permanent)
    ...
    energy = energy + (rarity.value + 1) * 20 -- add 0% (worst rarity) to +80% (best rarity)

    -- add randomized percentage, span is based on rarity
    energy = energy + math.random() * ((rarity.value + 1) * 15) -- add random value between 0% (worst rarity) and +60% (best rarity)
    energy = energy / 100

    local charge = 20 -- base value, in percent
    -- add flat percentage based on rarity
...
function getPrice(seed, rarity)
    ...
    local price = energy * 100 * 150 + charge * 100 * 100;
