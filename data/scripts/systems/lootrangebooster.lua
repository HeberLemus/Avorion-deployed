...
function getLootCollectionRange(seed, rarity, permanent)
    local range = (rarity.value + 2) * 20 * (1.3 ^ rarity.value) -- one unit is 10 meters
...
function getTooltipLines(seed, rarity, permanent)
    ...
    return
    {
        {ltext = "Loot Collection Range"%_t, rtext = "+${distance} km"%_t % {distance = range / 100}, icon = "data/textures/icons/coins.png", boosted = permanent}
    },
    {
        {ltext = "Loot Collection Range"%_t, rtext = "+${distance} km"%_t % {distance = baseRange * 2 / 100}, icon = "data/textures/icons/coins.png"}
    }
end
