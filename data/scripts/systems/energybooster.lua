...
function getBonuses(seed, rarity, permanent)
    ...
    energy = energy + (rarity.value + 1) * 100 -- add 0% (worst rarity) to +60% (best rarity)

    -- add randomized percentage, span is based on rarity
    energy = energy + math.random() * ((rarity.value + 1) * 8) -- add random value between 0% (worst rarity) and +48% (best rarity)
    energy = energy / 100

    local charge = 25 -- base value, in percent
    -- add flat percentage based on rarity
    charge = charge + (rarity.value + 1) * 5 -- add 0% (worst rarity) to +24% (best rarity)
...
function getPrice(seed, rarity)
    ...
    local price = energy * 100 * 400 + charge * 100 * 200
...
function getTooltipLines(seed, rarity, permanent)
    ...
    if charge ~= 0 then
        ...
        table.insert(bonuses, {ltext = "Recharge Rate"%_t, rtext = string.format("%+i%%", baseCharge * 0.5 * 100), icon = "data/textures/icons/energise.png"})
    end
