...
function getBonuses(seed, rarity, permanent)
    ...
    vfactor = vfactor + (rarity.value + 1) * 5 -- add 0% (worst rarity) to +18% (best rarity)

    -- add randomized percentage, span is based on rarity
    vfactor = vfactor + math.random() * ((rarity.value + 1) * 5) -- add random value between 0% (worst rarity) and +24% (best rarity)
    vfactor = vfactor / 100

    local afactor = 10 -- base value, in percent
    -- add flat percentage based on rarity
    afactor = afactor + (rarity.value + 1) * 7 -- add 0% (worst rarity) to +30% (best rarity)
