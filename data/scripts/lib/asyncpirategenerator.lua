...
function AsyncPirateGenerator:createScaledDisruptor(position, extra)
	local extra = extra or 1.0
    local scaling = self:getScaling()
		scaling = scaling * extra
    return self:create(position, 2.0 * scaling, .85 * extra, "Disruptor"%_T)
end
...
function AsyncPirateGenerator:createScaledRavager(position)
    ...
    local scaling = self:getScaling()
		scaling = scaling * extra
    return self:create(position, 8.0 * scaling, 1.0 * extra, "Ravager"%_T)
end
...
function AsyncPirateGenerator:createDisruptor(position)
    return self:create(position, 2.0, 1.35, "Disruptor"%_T)
end
...
function AsyncPirateGenerator:createRavager(position)
    return self:create(position, 8.0, 1.75, "Ravager"%_T)
end
